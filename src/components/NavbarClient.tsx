    ...
    <div className="navbarWrap">
      <nav className={`navbar ${isScrolled ? 'navbarScrolled' : ''}`}>
        ...
        <ul className="navLinks">
          ...
          {isAdmin && (
            <li><Link href="/admin" className="navLink" style={{ color: "var(--primary)" }}>Admin</Link></li>
          )}
        </ul>
              ...
              {open && (
                <div style={{
                  ...
                  <Link href="/upload" onClick={() => setOpen(false)} style={{
                    display: "block", padding: "10px 14px", fontSize: "0.9rem", borderRadius: "8px", color: "var(--text)",
                  }}>Upload Creation</Link>
                  {isAdmin && (
                    <Link href="/admin" onClick={() => setOpen(false)} style={{
                      display: "block", padding: "10px 14px", fontSize: "0.9rem", borderRadius: "8px", color: "var(--primary)",
                    }}>Admin Panel</Link>
                  )}
        ...
        {mobileMenuOpen && (
          <div className="mobileMenu">
            <div className="mobileMenuInner">
              ...
              {isAdmin && (
                <Link href="/admin" onClick={() => setMobileMenuOpen(false)} className="mobileNavLink" style={{ color: "var(--primary)" }}>
                  Admin Panel
                </Link>
              )}
              {session?.user ? (
