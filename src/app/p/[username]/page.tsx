      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "40px", alignItems: "start" }}>
        ...
        <div>
          <div style={{ marginBottom: "36px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "20px", marginBottom: "24px" }}>
              {user.image ? (
                <Image src={user.image} alt="" width={80} height={80} style={{ borderRadius: "50%" }} />
              ) : (
          ...
          <div style={{ display: "grid", gap: "18px", marginBottom: "30px" }}>
            {user.gamertag && (
              <div style={{ display: "grid", gap: "4px" }}>
                <span style={{ color: "var(--text-secondary)", fontSize: "0.9rem" }}>Gamertag</span>
                <strong style={{ fontSize: "1rem", color: "var(--primary)" }}>{user.gamertag}</strong>
              </div>
            )}
            {user.youtube && (
              <div style={{ display: "grid", gap: "4px" }}>
                <span style={{ color: "var(--text-secondary)", fontSize: "0.9rem" }}>YouTube</span>
                <a href={user.youtube} target="_blank" rel="noopener noreferrer" style={{ fontSize: "1rem", color: "var(--primary)" }}>
                  {user.youtube.replace("https://www.youtube.com/", "").replace("https://youtube.com/", "")}
                </a>
              </div>
            )}
            {user.twitch && (
              <div style={{ display: "grid", gap: "4px" }}>
                <span style={{ color: "var(--text-secondary)", fontSize: "0.9rem" }}>Twitch</span>
                <a href={`https://twitch.tv/${user.twitch}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: "1rem", color: "var(--primary)" }}>
                  {user.twitch}
                </a>
              </div>
            )}
        ...
        {user.gamertag && (
          <div style={{ position: "sticky", top: "120px" }}>
            <SkinViewer gamertag={user.gamertag} />
          </div>
        )}
      </div>
    </div>
